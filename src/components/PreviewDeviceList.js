...
const PreviewDeviceList = observer(
  ({ devices: explicitDevices, viewMode = "grid" }) => {
    ...
    const listWrapperClass =
      viewMode === "grid"
        ? "mt-8 grid grid-cols-2 gap-4 sm:grid-cols-2 sm:gap-5 xl:grid-cols-3"
              ...
              >
                ...
                >
                  {brokenImages[product.id] ? (
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 text-[var(--color-text-muted)]">
                      <PhotoIcon className="h-10 w-10 opacity-70" aria-hidden="true" />
                    </div>
                  ) : (
                    <img
