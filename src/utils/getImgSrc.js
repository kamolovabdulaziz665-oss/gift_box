...
export const getImgSrc = (img) => {
  ...
  // If it looks like a pathname (contains filename with extension), treat it as pathname
  if (img.includes('.') && !img.startsWith('/')) {
    return `${process.env.REACT_APP_API_URL}/api/blob?pathname=${encodeURIComponent(img)}`;
  }
